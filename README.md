...
또한 `flutter_local_notifications`와 `timezone`을 활용하여 약 복용 및 예방접종 일정에 따른 로컬 알림을 예약하고 관리할 수 있도록 구현했으며, 건강 기록 사진 첨부, 데이터 백업 및 복원, 기록 검색, 건강 요약 기능 등을 추가하여 실제 사용성을 높였습니다.
...
오늘 예정된 일정이 없는 경우에는 오늘의 건강 관리 영역을 별도로 표시하지 않도록 구성했습니다.
...
반려동물 상세 화면에서 `건강 요약 보기`를 통해 별도의 건강 요약 화면으로 이동할 수 있습니다.

건강 요약 화면에서는 반려동물의 주요 건강 데이터를 한눈에 확인할 수 있습니다.
...
건강 기록을 단순히 목록으로 확인하는 것뿐만 아니라 최근 상태와 주요 통계를 요약하여 확인할 수 있도록 구성했습니다.
...
새로운 기록을 추가할 경우 검색 모드는 자동으로 종료되도록 구성했습니다.
...
현재 데이터베이스 버전은 `9`이며, 건강 기록 사진을 별도 테이블로 관리하기 위한 `health_record_images` 테이블과 약 복용 완료 로그를 관리하기 위한 `medication_logs` 테이블이 포함되어 있습니다.
...
또한 새로운 체중 기록을 등록하거나 기존 기록을 수정하면 가장 최근 체중 기록을 반려동물의 현재 몸무게에 자동으로 반영하도록 구성했습니다.
...
반복 복용과 다음 복용일은 서로 동시에 설정되지 않도록 구성했습니다.
...
건강 데이터를 한눈에 확인할 수 있는 요약 기능을 추가하고, 실제 사용 테스트를 통해 일정 및 UI/UX를 개선했습니다.
...
현재 기본 기능과 데이터 관리 기능을 구현하고 실제 기기를 통한 테스트까지 진행했습니다.
...
Pet Care Manager Mobile 1.2.0 업데이트.
